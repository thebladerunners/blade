...
// Copyright (c) 2009-2012 The Bitcoin developers - Blade developers  2014 
...
const string strMessageMagic = "Blade Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    if(fTestNet || nTime > PROTOCOL_SWITCH_TIME)
    {
        ...
        // Blade: reward for coin-year is cut in half every 64x multiply of PoS difficulty
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Blade
    bool fStrictPayToScriptHash = true; // Always active in Blade
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Blade", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nModifierInterval = 7 * 60; // test modifier interval is 7 minutes
